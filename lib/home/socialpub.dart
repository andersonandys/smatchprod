...
class _SocialpubState extends State<Socialpub> {
                  ...
                  children: <Widget>[
                    ListTile(
                      leading: CircleAvatar(
                        radius: 30,
                        backgroundImage:
                            CachedNetworkImageProvider(avatarcompte),
                      ),
                      title: Text(
                        nomcomptepub,
                ...
                children: <Widget>[
                  ...
                  if (typecontenu == "image")
                    Column(
                      ...
                      children: <Widget>[
                        Text(
                          "Image traitee $nbresend/ ${imageselect.length}",
                          style: const TextStyle(
                              color: Colors.white, fontSize: 18),
                        ),
                        IconButton(
                            onPressed: () {
                              sendmedia("idpublication");
                            },
                            icon: Icon(
                              Icons.send,
                              color: Colors.white,
  ...
  sendmedia(idpublication) {
    for (String element in imageselect) {
      ...
      task.whenComplete(() => upload("image", idpublication, element));
      setState(() {
        nbre++;
        nbresend++;
      });
  ...
  Future<void> upload(type, valueid, name) async {
    String downloadURL = await FirebaseStorage.instance
        ...
        .getDownloadURL();
    if (type == "image") {
      // instance.collection("ab").add({type: downloadURL});
      instance
          .collection("publication")
          .doc(valueid)
          .collection(type)
